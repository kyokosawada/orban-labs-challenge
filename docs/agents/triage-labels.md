...
## Label state on GitHub

As of setup, `wontfix` already exists on the repo (it ships with GitHub's default label set). The other four — `needs-triage`, `needs-info`, `ready-for-agent`, `ready-for-human` — do not exist yet and need creating before first use:
