...
def test_a_keyword_narrows_the_listing_to_the_notes_mentioning_it(client):
...
def test_a_keyword_is_looked_for_in_the_body_as_well_as_the_title(client):
...
def test_a_keyword_matches_wherever_it_appears_rather_than_at_the_start(client):
    create_note(client, title="Chase the invoice")

    assert titles_found(client, q="invoice") == ["Chase the invoice"]


def test_a_keyword_ignores_capitalisation(client):
...
def test_a_keyword_and_a_tag_narrow_together_rather_than_apart(client):
...
def test_a_keyword_and_a_tag_that_share_no_note_find_nothing(client):
    create_note(client, title="Invoice for March", tags=["work"])
...
def test_an_absent_keyword_returns_the_whole_listing(client):
    create_note(client, title="Invoice")
    create_note(client, title="Standup")
...
def test_an_empty_keyword_returns_the_whole_listing_rather_than_nothing(client):
    create_note(client, title="Invoice")
    create_note(client, title="Standup")
...
def test_surrounding_spaces_around_a_keyword_are_ignored(client):
    create_note(client, title="Invoice")

    assert titles_found(client, q="  invoice  ") == ["Invoice"]


def test_a_keyword_no_note_mentions_finds_nothing_rather_than_everything(client):
    create_note(client, title="Invoice")

    assert titles_found(client, q="gardening") == []


def test_a_wildcard_is_looked_for_literally_rather_than_matching_anything(client):
...
def test_the_notes_a_keyword_finds_are_still_most_recently_changed_first(client):
    create_note(client, title="Invoice for March")
...
def test_a_note_found_by_a_keyword_still_carries_its_tags(client):
...
def test_a_deleted_note_is_not_found_by_a_keyword(client):
