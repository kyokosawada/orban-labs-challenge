def test_a_note_survives_a_create_and_is_read_back_in_the_listing(client, create_note):
...
def test_a_note_can_be_written_with_a_title_alone(client, create_note):
...
def test_a_title_is_stored_without_its_surrounding_spaces(client, create_note):
    created = create_note(client, title="   Padded   ")

    assert created["title"] == "Padded"


def test_a_note_carries_the_times_it_was_created_and_last_changed(client, create_note):
...
def test_notes_are_listed_most_recently_changed_first(client, create_note):
...
def test_notes_survive_a_restart_because_schema_creation_is_idempotent(
    client, build_client, create_note
):
